...
class SkjermetPersonPolicyImplTest {

	private val adGruppeProvider = mockk<AdGruppeProvider>()

	private val policy = SkjermetPersonPolicyImpl(adGruppeProvider)

	@Test
	fun `should return "permit" if access to 0000-ga-TODO`() {
		val navIdent = "Z1234"

		every {
			adGruppeProvider.hentAdGrupper(navIdent)
		} returns listOf(
			AdGruppe(UUID.randomUUID(), AdGrupper.SKJERMET_PERSON),
			AdGruppe(UUID.randomUUID(), "some-other-group"),
		)

		val decision = policy.evaluate(SkjermetPersonPolicy.Input(navIdent))

		decision shouldBe Decision.Permit
	}

	@Test
	fun `should return "deny" if not access to 0000-ga-TODO`() {
		val navIdent = "Z1234"
		...
		if (decision is Decision.Deny) {
			decision.message shouldBe "NAV ansatt mangler tilgang til AD gruppen 0000-ga-TODO"
